// =============================================================================
// deveco-session-meta-store.ts — Deveco Code 会话 sessionId → meta 持久化映射
// =============================================================================
...
export const DEVECO_SESSION_META_FILE = join(
  USER_DATA_DIR,
  "state",
  "deveco-session-meta.json",
);

export interface DevecoSessionMeta {
  cwd: string;
  /** OpenCode / Deveco Code 原生 session ID，用于 run --session 恢复 */
  threadId?: string;
}

export interface DevecoSessionMetaStore {
  get(sessionId: string): Promise<DevecoSessionMeta | undefined>;
  set(sessionId: string, partial: Partial<DevecoSessionMeta>): Promise<void>;
...
export function createDevecoSessionMetaStore(
  filePath: string = DEVECO_SESSION_META_FILE,
): DevecoSessionMetaStore {
  ...
  async function save(map: Record<string, RawEntry>): Promise<void> {
    try {
      ...
      console.error(
        `[deveco-session-meta] failed to persist ${filePath}: ${(err as Error).message}`,
      );
    }
  }

  return {
    async get(sessionId: string): Promise<DevecoSessionMeta | undefined> {
    ...
    async set(
      sessionId: string,
      partial: Partial<DevecoSessionMeta>,
    ): Promise<void> {
      const map = await load();
...
export const defaultDevecoSessionMetaStore = createDevecoSessionMetaStore();
